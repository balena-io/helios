...
pub struct Host {
    ...
    pub engine_runtimes: Vec<String>,
}
...
fn derive_overlay_status(
    ...
    match state.status {
        ...
        ContainerStatus::Dead => return None,
        // 126/127: the runtime could not exec the command.
        ContainerStatus::Failed(126 | 127) => return Some(OverlayStatus::Failed),
        // Any other start failure is engine-side and retryable.
        ContainerStatus::Created | ContainerStatus::Failed(_) => return None,
        // A clean one-shot exit is the only activation the overlay gets.
        ContainerStatus::Stopped(0) => {}
        _ => return Some(OverlayStatus::Failed),
    }
...
/// a container left behind by an interrupted deploy. It also returns `None` for
/// an overlay of ours whose start failed for an engine-side reason that
/// `derive_overlay_status` does not classify.
...
mod tests {
    ...
    fn derives_active_when_staged_in_an_earlier_boot() {
        let st = state(ContainerStatus::Stopped(0));
        assert_eq!(
    ...
    fn derives_failed_on_nonzero_exit() {
        let st = state(ContainerStatus::Stopped(1));
        assert_eq!(
            derive_overlay_status(&st, Some(OTHER_BOOT), THIS_BOOT, None, None),
    ...
    fn derives_failed_when_the_container_is_still_running() {
        ...
        let st = state(ContainerStatus::Running);
        assert_eq!(
            derive_overlay_status(&st, Some(OTHER_BOOT), THIS_BOOT, None, None),
            Some(OverlayStatus::Failed)
        );
    ...
    fn reports_no_status_for_a_container_that_never_started() {
        ...
        let st = state(ContainerStatus::Created);
        assert_eq!(
            derive_overlay_status(&st, Some(THIS_BOOT), THIS_BOOT, None, None),
            None
        );
    ...
    fn reports_no_status_for_a_container_the_removal_left_dead() {
        ...
        // same overlay.
        let st = state(ContainerStatus::Dead);
        assert_eq!(
            derive_overlay_status(&st, Some(OTHER_BOOT), THIS_BOOT, None, None),
            None
        );
    ...
    fn derives_failed_when_the_runtime_refused_the_start() {
        ...
        let st = state(ContainerStatus::Failed(127));
        assert_eq!(
            derive_overlay_status(&st, Some(THIS_BOOT), THIS_BOOT, None, None),
            Some(OverlayStatus::Failed)
        );
    ...
    fn derives_failed_when_the_runtime_could_not_run_the_command() {
        ...
        let st = state(ContainerStatus::Failed(126));
        assert_eq!(
            derive_overlay_status(&st, Some(THIS_BOOT), THIS_BOOT, None, None),
            Some(OverlayStatus::Failed)
        );
    ...
    fn an_engine_side_start_failure_stays_retryable() {
        // Any other code is engine-side and transient.
        let st = state(ContainerStatus::Failed(1));
        assert_eq!(
            derive_overlay_status(&st, Some(THIS_BOOT), THIS_BOOT, None, None),
            None
        );
