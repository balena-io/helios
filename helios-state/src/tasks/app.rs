...
/// True if the release exists and is already marked as installed
...
/// Check if a service start should be skipped because one of its dependencies cannot be met
///
/// Used as a planning exception to abort in this scenario instead of failing to find a plan
...
/// The container an optional wait ended on, warning first if its condition can
/// no longer be met. Compose starts a dependent regardless, so the run carries
/// on either way.
fn warn_if_unmet(outcome: WaitOutcome) -> Container {
    match outcome {
        WaitOutcome::Met(observed) => observed,
        WaitOutcome::Unmet(reason, observed) => {
