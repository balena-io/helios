    ...
    deps.into_iter()
        ...
        .filter(|(dep_name, spec)| {
            services
                ...
                .is_some_and(|dep| dep.started && !condition_met(dep, spec.condition))
        })
...
fn await_healthy(
    ...
    let container_id = format!("{service_name}_{commit}");
    ...
    with_io(status, async move |status| {
        ...
        loop {
            let local = docker
                ...
                .await
                .with_context(|| {
                    "failed to inspect container while awaiting completion".to_string()
                })?;
            let observed = Container::from((container_id.as_str(), local.state));
            // exit code is only meaningful once the container has stopped
            if observed.status == ContainerStatus::Stopped {
                match observed.exit_code {
                    Some(0) => break,
                    Some(code) => {
                        return Err(format!(
                            "dependency failed while awaiting completion: exited with code {code}"
                        )
                        .into());
                    }
                    None => {}
