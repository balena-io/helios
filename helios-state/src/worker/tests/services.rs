...
// One service depended on under two different conditions emits a separate await
// per condition.
//
// TODO: both starts are gated on both awaits, so 'on-healthy' waits for the
// completion await it does not depend on. mahler chains each planning level
// through a single join, so a start cannot attach to one await branch. Asserted
// as-is to pin current behaviour; the plan-ordering fix will update it.
#[test]
fn it_emits_separate_awaits_for_a_shared_dependency_under_two_conditions() {
    init_tracing();
    assert_workflow(
        json!({
            "uuid": "my-device-uuid",
                        ...
                        "my-release-uuid": {
                            ...
                            "services": {
                                ...
                                "on-healthy": {
                                    ...
                                    "started": true,
                                    "depends_on": {
                                        "shared": {"condition": "service_healthy", "restart": false, "required": true}
                                    },
                                    "config": {},
                                },
                                "on-completed": {
                                    ...
                                    "started": true,
                                    "depends_on": {
                                        "shared": {"condition": "service_completed_successfully", "restart": false, "required": true}
                                    },
                                    "config": {},
                                },
                            }
        ...
        seq!("initialize release 'my-release-uuid' for app with uuid 'my-app-uuid'")
            + par!(
                "initialize service 'on-completed' for release 'my-release-uuid'",
                "initialize service 'on-healthy' for release 'my-release-uuid'",
