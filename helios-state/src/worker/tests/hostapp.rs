...
fn it_defers_the_overlay_reboot_while_validation_runs() {
    ...
    // the rollback. The reboot task enforces on the validation flag, so the
    // planner sequences the wait first, and the wait fails.
    init_tracing();
    assert_workflow(
        json!({
            "name": "device-name",
...
#[test]
fn it_redeploys_an_overlay_created_against_another_runtime() {
    // A container keeps its creation runtime, so a change needs a recreate.
    init_tracing();
    assert_workflow(
        json!({
            "name": "device-name",
...
fn it_redeploys_an_overlay_whose_kernel_did_not_boot() {
    ...
    // remedy is the deploy path, which re-runs activation and re-arms.
    init_tracing();
    assert_workflow(
        json!({
            "name": "device-name",
...
fn it_reboots_for_a_removal_while_a_pre_extension_host_holds_a_declined_release() {
    ...
    assert_workflow(
        ...
        // seek prunes the release before the worker sees it.
        json!({
            "name": "device-name",
            "uuid": "my-device-uuid",
            "host": {
