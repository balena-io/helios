...
impl Healthcheck {
    ...
    pub fn time_to_unhealthy(&self) -> Duration {
...
mod tests {
    ...
    #[test]
    fn healthcheck_time_to_unhealthy() {
        ...
        assert_eq!(hc.time_to_unhealthy(), Duration::from_secs(10 + 4 * 7));

        // engine defaults: no grace period, 3 retries of a 30s probe every 30s
        let hc = Healthcheck::default();
        assert_eq!(hc.time_to_unhealthy(), Duration::from_secs(3 * 60));
        ...
        assert_eq!(hc.time_to_unhealthy(), Duration::MAX);
    }
