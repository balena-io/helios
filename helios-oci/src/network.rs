...
impl NetworkClient<'_> {
    ...
    pub async fn create(&self, name: &str, config: NetworkConfig) -> Result<()> {
        ...
        match self.0.inner().create_network(request).await {
            Ok(_) => Ok(()),
            Err(e) => {
                Err(Error::from(e).context(format!("failed to create network {name}")))
            }
        }
    }
    ...
    pub async fn remove(&self, name: &str) -> Result<()> {
        match self.0.inner().remove_network(name).await {
            ...
            Err(e) => {
                Err(Error::from(e).context(format!("failed to remove network {name}")))
            }
        }
    }
...
#[derive(Debug, Clone)]
pub struct IpamDriver(String);

impl Default for IpamDriver {
    fn default() -> Self {
        Self("default".to_string())
    }
}

impl fmt::Display for IpamDriver {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(f)
    }
}

impl From<String> for IpamDriver {
    fn from(s: String) -> Self {
        Self(s)
    }
}
...
impl TryFrom<NetworkInspect> for LocalNetwork {
    ...
    fn try_from(value: NetworkInspect) -> Result<Self> {
        ...
        let driver = NetworkDriver::from(
            value.driver.unwrap_or_else(|| "bridge".to_string()),
        );
