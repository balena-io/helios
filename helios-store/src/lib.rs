...
//! store.put("my/document", &json!({ "hello": "world" })).await?;
//! # Ok(())
//! # }
//! ```
...
impl Error {
    ...
    fn with_path(path: &Path) -> impl Fn(io::Error) -> Self {
        |err| match err.downcast::<Self>() {
            Ok(Self::Io { source, .. }) => Self::Io {
                path: Some(path.to_path_buf()),
                source,
            },
            Ok(Self::NotFound { .. }) => Self::NotFound {
...
impl From<io::Error> for Error {
    fn from(value: io::Error) -> Self {
        match value.downcast::<Self>() {
            // because of run_async sometimes we wrap the Error
            // in an std::io::Error for correct typing. We check
            // for that scenario here when converting back from an io::Error
            Ok(err) => err,
            Err(io_err) => match io_err.downcast::<serde_json::Error>() {
                Ok(e) => Self::from(e),
                Err(io_err) => Self::Io {
                    path: None,
                    source: io_err,
