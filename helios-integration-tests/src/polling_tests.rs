...
// Synthetic: no kernel build hashes to this value.
const TEST_KERNEL_ABI_ID: &str = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
...
/// Build and push an overlay image whose activation fails: no `/boot` backs
/// its kernel claim.
async fn build_failing_overlay_image(docker: &Docker) {
    let dockerfile =
        format!("FROM alpine:3.23\nLABEL io.balena.image.kernel-abi-id={TEST_KERNEL_ABI_ID}\n");
    ...
    releases.insert(
        ...
        json!({
            "services": {
                ...
                "kernel-modules": {
                    "id": 402,
                    // Its image label carries a claim alpine cannot back.
