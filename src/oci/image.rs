...
impl Image<'_> {
    ...
    pub async fn tag(&self, name: &str, new_name: &ImageUri) -> Result<()> {
        ...
        res.map_err(Error::from)
            .with_context_lazy(|| format!("failed to tag image {name} as {}", new_name.as_str()))?;
    ...
    pub async fn pull(&self, image: &ImageUri, creds: Option<Credentials>) -> Result<()> {
        ...
        while let Some(progress) = stream.next().await {
            ...
            let _ = progress
                .map_err(Error::from)
                .with_context_lazy(|| format!("failed to pull image {}", image.as_str()))?;
        }
    ...
    pub async fn inspect(&self, image: &ImageUri) -> Result<LocalImage> {
        ...
        let info = res
            .map_err(Error::from)
            .with_context_lazy(|| format!("failed to inspect image {}", image.as_str()))?;
    ...
    pub async fn remove(&self, image: &ImageUri) -> Result<()> {
        self.0
            ...
            .await
            .map_err(Error::from)
            .with_context_lazy(|| format!("failed to remove image {}", image.as_str()))?;
