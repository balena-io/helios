...
/// Example:
/// ```rust,no_run
/// local::store("/apps/{10c41}", "my-app-name").await?;
/// ```
...
/// Example:
/// ```rust,no_run
/// local::store_with_name("/apps/{10c41}", "name", "my-app-name").await?;
/// ```
...
pub async fn read<P: AsRef<Path>, V: DeserializeOwned>(
    path: P,
) -> Result<Option<V>, LocalStateError> {
    let local_path = with_state_dir(path);
