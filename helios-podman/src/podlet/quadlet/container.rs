...
use crate::podlet::serde::{quadlet::seq_quote_whitespace, serialize_display_seq, skip_true};
...
impl Container {
    ...
    fn push_args(&mut self, args: impl Serialize) -> Result<(), crate::podlet::serde::args::Error> {
        let args = crate::podlet::serde::args::to_string(args)?;
        if !args.is_empty() {
...
mod tests {
    ...
    #[test]
    fn container_default_empty() -> Result<(), crate::podlet::serde::quadlet::Error> {
        let container = Container {
            image: String::from("image"),
            ..Container::default()
        };
        assert_eq!(
            crate::podlet::serde::quadlet::to_string_join_all(container)?,
            "[Container]\nImage=image\n"
        );
        Ok(())
    }
