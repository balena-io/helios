...
pub use self::{
    container::Container, globals::Globals, install::Install, network::Network, service::Service,
    unit::Unit, volume::Volume,
};
use crate::podlet::serde::skip_true;
...
impl File {
    ...
    ) -> Result<String, crate::podlet::serde::quadlet::Error> {
        crate::podlet::serde::quadlet::to_string(self, join_options)
    }
...
pub enum JoinOption {
    ...
    /// `Annotation=`, used in [Container] sections.
    Annotation,
    ...
    /// `Environment=`, used in [Container] sections.
    Environment,

    /// `Label=`, used in [Container], [Network], and [Volume] sections.
    Label,
...
impl Resource {
    ...
    pub fn name_to_service(&self, name: &str) -> String {
        let mut service = match self {
            Self::Container(_) => String::from(name),
...
enum ResourceIter<C, N, V> {
    Container(C),
    Network(N),
    Volume(V),
}

impl<C, N, V, Item> Iterator for ResourceIter<C, N, V>
where
...
impl FromStr for PodmanVersion {
    type Err = ParsePodmanVersionError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "4.4" | "4.4.0" | "4.4.1" | "4.4.2" | "4.4.3" | "4.4.4" => Ok(Self::V4_4),
            "4.5" | "4.5.0" | "4.5.1" => Ok(Self::V4_5),
            "4.6" | "4.6.0" | "4.6.1" | "4.6.2" => Ok(Self::V4_6),
            "4.7" | "4.7.0" | "4.7.1" | "4.7.2" => Ok(Self::V4_7),
            "4.8" | "4.8.0" | "4.8.1" | "4.8.2" | "4.8.3" | "4.9" | "4.9.0" | "4.9.1" | "4.9.2"
            | "4.9.3" | "4.9.4" | "4.9.5" => Ok(Self::V4_8),
            "5.0" | "5.0.0" | "5.0.1" | "5.0.2" | "5.0.3" => Ok(Self::V5_0),
            "5.1" | "5.1.0" | "5.1.1" | "5.1.2" => Ok(Self::V5_1),
            "5.2" | "5.2.0" | "5.2.1" | "5.2.2" | "5.2.3" | "5.2.4" | "5.2.5" => Ok(Self::V5_2),
            "5.3" | "5.3.0" | "5.3.1" | "5.3.2" => Ok(Self::V5_3),
            "5.4" | "5.4.0" | "5.4.1" | "5.4.2" => Ok(Self::V5_4),
            "5.5" | "5.5.0" | "5.5.1" | "5.5.2" => Ok(Self::V5_5),
            "5.6" | "5.6.0" | "5.6.1" | "5.6.2" | "latest" => Ok(Self::V5_6),
            "5.7" | "5.7.0" | "5.7.1" => Ok(Self::V5_7),
            "5.8" | "5.8.0" | "5.8.1" => Ok(Self::V5_8),
            s => Err(ParsePodmanVersionError(s.to_owned())),
        }
    }
}

/// Error returned when attempting to parse an invalid [`PodmanVersion`] variant.
#[derive(Debug, Error)]
#[error("unknown Podman version `{0}`")]
pub struct ParsePodmanVersionError(String);
...
impl AutoUpdate {
    ...
    pub(crate) const LABEL_KEY: &'static str = "io.containers.autoupdate";
...
///
/// Prefer using [`push_arg()`] as it quotes whitespace.
