...
use crate::podlet::quadlet::JoinOption;
...
impl Display for QuoteWhitespace<'_> {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        if self.0.contains(char::is_whitespace) || self.0.contains('"') {
            f.write_char('"')?;
...
mod tests {
    ...
    fn quote_whitespace() {
        ...
        let quoted = QuoteWhitespace("noquotes");
        assert_eq!(quoted.to_string(), "noquotes");
    }
}
